...
def launch_training_task_v2(
    dataset_two_trigger_target: torch.utils.data.Dataset,
    dataset_text_trigger_target: torch.utils.data.Dataset,
    dataset_image_trigger_target: torch.utils.data.Dataset,
    dataset_no_trigger_origin: torch.utils.data.Dataset,
    dataset_text_trigger_origin: torch.utils.data.Dataset,
    dataset_image_trigger_origin: torch.utils.data.Dataset,
    model: DiffusionTrainingModule,
    model_logger: ModelLogger,
    learning_rate: float = 1e-5,
    weight_decay: float = 1e-2,
    ...
    optimizer = torch.optim.AdamW(model.trainable_modules(), lr=learning_rate, weight_decay=weight_decay)
    scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer)
    dataloader1 = torch.utils.data.DataLoader(dataset_two_trigger_target, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    dataloader2 = torch.utils.data.DataLoader(dataset_text_trigger_target, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    dataloader3 = torch.utils.data.DataLoader(dataset_image_trigger_target, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    dataloader4 = torch.utils.data.DataLoader(dataset_no_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    dataloader5 = torch.utils.data.DataLoader(dataset_text_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    dataloader6 = torch.utils.data.DataLoader(dataset_image_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers)
    ...
    for epoch_id in range(num_epochs):
        for data1, data2, data3, data4, data5, data6 in tqdm(zip(dataloader1, dataloader2, dataloader3, dataloader4, dataloader5, dataloader6), total=total_steps):
            ...
            with contextlib.nullcontext():
                optimizer.zero_grad()
                if getattr(dataset_two_trigger_target, "load_from_cache", False):
...
def launch_training_task_v3(
    ...
    dataloader1 = torch.utils.data.DataLoader(
        dataset_two_trigger_target, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers
    )
    dataloader4 = torch.utils.data.DataLoader(
        dataset_no_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers
    )
    dataloader5 = torch.utils.data.DataLoader(
        dataset_text_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers
    )
    dataloader6 = torch.utils.data.DataLoader(
        dataset_image_trigger_origin, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers
    )
    ...
    # 根据 loss_weight_type 选择权重分配方式
    loss_weight_type = getattr(args, "loss_weight_type", "gradnorm")
    
    if loss_weight_type == "fixed":
        # 固定权重模式：从 args 读取静态权重值
        fixed_weights = torch.tensor([
            getattr(args, "loss1_weight_value", 1.0),
            getattr(args, "loss4_weight_value", 1.0),
            getattr(args, "loss5_weight_value", 1.0),
            getattr(args, "loss6_weight_value", 1.0),
        ], dtype=torch.bfloat16)
        gradnorm = None
    elif loss_weight_type == "gradnorm":
        # 动态权重模式：使用 GradNorm
        gradnorm = GradNormTracker(num_tasks=4, alpha=0.5)
        fixed_weights = None
    else:
        raise ValueError(f"Unsupported loss_weight_type: {loss_weight_type}. Choose 'fixed' or 'gradnorm'.")
...
def launch_training_task_v4(
    ...
    """
    多任务扩散模型训练函数（v4版本），采用顺序forward+立即backward策略降低显存峰值
    
    Args:
        dataset_two_trigger_target (torch.utils.data.Dataset): 双触发目标数据集
        dataset_no_trigger_origin (torch.utils.data.Dataset): 无触发源数据集  
        dataset_text_trigger_origin (torch.utils.data.Dataset): 文本触发源数据集
        dataset_image_trigger_origin (torch.utils.data.Dataset): 图像触发源数据集
        model (DiffusionTrainingModule): 待训练的扩散模型
        model_logger (ModelLogger): 模型日志记录器
        learning_rate (float, optional): 学习率. Defaults to 1e-5.
        weight_decay (float, optional): 权重衰减. Defaults to 1e-2.
        num_workers (int, optional): 数据加载线程数. Defaults to 1.
        save_steps (int, optional): 模型保存步数间隔. Defaults to None.
        num_epochs (int, optional): 训练轮数. Defaults to 1.
        args (Any, optional): 命令行参数对象. Defaults to None.
    
    Note:
        - 采用GradNorm算法自动平衡多任务权重
        - 支持Weights & Biases实验跟踪(可选)
        - 显存优化策略：分阶段计算+立即释放计算图
    """
    ...
    for epoch_id in range(num_epochs):
        for data1, data4, data5, data6 in tqdm(
            ...
            total=total_steps,
                ...
                # 根据权重类型获取权重
                if loss_weight_type == "fixed":
                    ws = fixed_weights.to(l1.device)
                    weights_cpu = fixed_weights.detach().cpu().tolist()
                else:  # gradnorm
                    ws = gradnorm.update([l1, l4, l5, l6]).to(l1.device)
                    weights_cpu = gradnorm.weights.detach().cpu().tolist()
...
def launch_training_task_w_con_v3(
    ...
    for epoch_id in range(num_epochs):
        for data1, data2, data3, data4 in tqdm(zip(dataloader1, dataloader2, dataloader3, dataloader4), total=total_steps):
            ...
            with contextlib.nullcontext():
                optimizer.zero_grad()
                if dataset_two_trigger_target.load_from_cache:
                    loss1 = model({}, inputs=data1)
                    (w1 * loss1).backward()
...
def launch_training_task_w_con_v5(
    dataset_two_trigger_target: torch.utils.data.Dataset,# 图文双trigger-目标图像
    dataset_text_trigger_target: torch.utils.data.Dataset,# 文本trigger-目标图像
    dataset_image_trigger_target: torch.utils.data.Dataset,# 图像trigger-目标图像
    dataset_no_trigger_origin: torch.utils.data.Dataset,# no trigger-原始图像
    dataset_text_trigger_origin: torch.utils.data.Dataset,# 文本trigger-原始图像
    dataset_image_trigger_origin: torch.utils.data.Dataset,# 图像trigger-原始图像
    model: DiffusionTrainingModule,
    model_logger: ModelLogger,
    learning_rate: float = 1e-5,
    weight_decay: float = 1e-2,
